...
describe('PUT /tags/:id', () => {
    ...
    it('updates existing tag successfully (200)', async () => {
        ...
        const newSize = Buffer.from(newNonce, 'base64').length + Buffer.from(newCiphertext, 'base64').length;
    ...
    it('returns 401 with invalid token', async () => {
        ...
        const res = await client.tags[':id'].$put(
            ...
            { headers: { Authorization: 'Bearer invalid_token', 'If-Match': currentEtag } }
        );
    ...
    it('returns 413 when item exceeds max size', async () => {
        ...
        const largeSize = Buffer.from(largeNonce, 'base64').length + Buffer.from(largeCiphertext, 'base64').length;
        const newUpdatedAt = Date.now();
    ...
    it('returns 404 when vault does not exist', async () => {
        ...
        const res = await client.tags[':id'].$put(
            ...
            { headers: { Authorization: `Bearer ${bobToken}`, 'If-Match': currentEtag } }
        );
    ...
    it('handles size mismatch gracefully (warning, not error)', async () => {
        const newNonce = Buffer.alloc(24, 2).toString('base64');
        const newCiphertext = Buffer.from('updated-tag-content').toString('base64');
        const actualSize = Buffer.from(newNonce, 'base64').length + Buffer.from(newCiphertext, 'base64').length;
        ...
        const res2 = await client.tags[':id'].$put(
            {
                ...
                json: {
                    ...
                    size: Buffer.from(Buffer.alloc(24, 3).toString('base64'), 'base64').length + Buffer.from(Buffer.from('updated-again').toString('base64'), 'base64').length,
                    updated_at: Date.now(),
                }
            },
