...
describe('POST /tags', () => {
    ...
    it('creates a tag and returns metadata (201)', async () => {
        ...
        const size = Buffer.from(nonce, 'base64').length + Buffer.from(ciphertext, 'base64').length;
        const now = Date.now();
    ...
    it('returns 409 on duplicate tag_id', async () => {
        ...
        const res1 = await client.tags.$post({ json: body }, { headers: { Authorization: `Bearer ${token}` } });
        expect(res1.status).toBe(201);

        const res2 = await client.tags.$post({ json: body }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    it('accepts optional tag_token field', async () => {
        ...
        const ciphertext = Buffer.from('encrypted-tag-with-token').toString('base64');
        const size = Buffer.from(nonce, 'base64').length + Buffer.from(ciphertext, 'base64').length;
        const now = Date.now();
    ...
    it('accepts null tag_token field', async () => {
        ...
        const ciphertext = Buffer.from('encrypted-tag-null-token').toString('base64');
        const size = Buffer.from(nonce, 'base64').length + Buffer.from(ciphertext, 'base64').length;
        const now = Date.now();
    ...
    it('handles different users independently', async () => {
        ...
        const size1 = Buffer.from(nonce1, 'base64').length + Buffer.from(ciphertext1, 'base64').length;
        const now = Date.now();
        ...
        const size2 = Buffer.from(nonce2, 'base64').length + Buffer.from(ciphertext2, 'base64').length;
    ...
    it('handles size mismatch gracefully', async () => {
        ...
        const actualSize = Buffer.from(nonce, 'base64').length + Buffer.from(ciphertext, 'base64').length;
