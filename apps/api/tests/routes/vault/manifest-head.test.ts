...
describe('HEAD /vault/manifest', () => {
    ...
    async function createManifest(version: number) {
        ...
        const ciphertext = Buffer.from('encrypted_manifest_data_v' + version).toString('base64');
    ...
    it('should return same ETag as GET request', async () => {
        ...
        expect(headRes.headers.get('x-vault-version')).toBe(getData.version.toString());
    });
    ...
    it('should work with multiple sessions for same user', async () => {
        ...
        expect(head1Res.headers.get('x-vault-version')).toBe(head2Res.headers.get('x-vault-version'));
    });
