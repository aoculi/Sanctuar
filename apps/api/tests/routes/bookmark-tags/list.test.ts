...
describe('GET /bookmarks/:id/tags', () => {
    ...
    beforeEach(async () => {
        ...
        const ciphertext = Buffer.from('encrypted-bookmark-content').toString('base64');
        const nonceWrap = Buffer.alloc(24, 2).toString('base64');
        const dekWrapped = Buffer.alloc(32, 3).toString('base64');
        const size = Buffer.from(nonce, 'base64').length + Buffer.from(ciphertext, 'base64').length + Buffer.from(nonceWrap, 'base64').length + Buffer.from(dekWrapped, 'base64').length;
        const now = Date.now();
        ...
        for (const t of tagIds) {
            ...
            const tSize = Buffer.from(tNonce, 'base64').length + Buffer.from(tCipher, 'base64').length;
