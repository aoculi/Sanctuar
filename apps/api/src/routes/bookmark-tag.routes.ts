...
import { createBookmarkTagLink, deleteBookmarkTagLink } from '../services/bookmark-tag.service';
...
bookmarkTagsRouter.post(
    '/',
    requireAuth,
    zValidator('json', createBookmarkTagLinkSchema, createValidationErrorHandler()),
    async (c) => {
        try {
...
bookmarkTagsRouter.delete(
    '/',
    requireAuth,
    zValidator('json', deleteBookmarkTagLinkSchema, createValidationErrorHandler()),
    async (c) => {
        try {
            // Get user ID from authenticated context (attached by middleware)
            const userId = (c.req.raw as any).userId as string;
