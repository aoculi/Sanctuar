...
const authSchema = z.object({
    login: z
        .string()
        .min(config.validation.login.minLength, `Login must be at least ${config.validation.login.minLength} characters`)
        .max(config.validation.login.maxLength, `Login must not exceed ${config.validation.login.maxLength} characters`)
        .trim(),
    password: z
        .string()
        .min(config.validation.password.minLength, `Password must be at least ${config.validation.password.minLength} characters`)
        .max(config.validation.password.maxLength, `Password must not exceed ${config.validation.password.maxLength} characters`),
});
