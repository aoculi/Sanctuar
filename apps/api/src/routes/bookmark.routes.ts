...
import { createBookmark, deleteBookmark, getBookmark, listBookmarks, updateBookmark } from '../services/bookmark.service';
 ...
 *     "ciphertext_content": "<base64>",   // AEAD content (url,title,notes,tags,...)
...
bookmarksRouter.get('/:id/tags', requireAuth, async (c) => {
    try {
        ...
        console.error('List tags for bookmark error:', error instanceof Error ? error.message : 'Unknown error');
