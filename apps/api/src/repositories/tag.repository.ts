 ...
 */
export async function tagExists(tagId: string, vaultId: string): Promise<boolean> {
 ...
 */
export async function listTags(vaultId: string, includeDeleted: boolean = false) {
...
export async function listTagsPaginated(
    ...
    const { limit, cursor, includeDeleted = false, updatedAfter, byToken } = options;
