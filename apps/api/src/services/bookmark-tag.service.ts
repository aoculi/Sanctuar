...
export const createBookmarkTagLink = async (
    ...
    try {
        ...
        const bookmark = await bookmarkRepository.findBookmarkById(input.item_id, vault.vaultId);
        ...
        console.log(`Bookmark-tag link created: item_id=${input.item_id}, tag_id=${input.tag_id}`);
        ...
        console.error('Create bookmark-tag link failed:', error instanceof Error ? error.message : 'Unknown error');
...
export const deleteBookmarkTagLink = async (
    ...
    try {
        ...
        console.log(`Bookmark-tag link deleted: item_id=${input.item_id}, tag_id=${input.tag_id}`);
        ...
        // Log error without sensitive data
        console.error('Delete bookmark-tag link failed:', error instanceof Error ? error.message : 'Unknown error');
...
export const listTagsForBookmark = async (
    ...
    try {
        ...
        const tagIds = await bookmarkTagRepository.listTagIdsForItem(vault.vaultId, itemId);
        ...
        console.error('List tags for bookmark failed:', error instanceof Error ? error.message : 'Unknown error');
