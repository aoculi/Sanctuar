 ...
 * @returns Vault metadata
 */
export const getVaultMetadata = async (userId: string): Promise<VaultMetadataOutput> => {
    try {
        ...
        console.error('Get vault metadata failed:', error instanceof Error ? error.message : 'Unknown error');
...
export const getManifest = async (userId: string): Promise<ManifestOutput> => {
    try {
        ...
        // Log error without sensitive data
        console.error('Get manifest failed:', error instanceof Error ? error.message : 'Unknown error');
...
export const upsertManifest = async (
    ...
    try {
        ...
        try {
            ...
            if (nonceBuffer.toString('base64') !== input.nonce ||
        ...
        const currentManifest = await vaultRepository.getManifestByVaultId(vault.vaultId);
        ...
        // Compute new ETag
        const newEtag = computeEtag(vault.vaultId, input.version, nonceBuffer, ciphertextBuffer);
        ...
        console.log(`Manifest ${isFirstWrite ? 'created' : 'updated'} for vault: ${vault.vaultId}, version: ${input.version}`);
        ...
        if (error instanceof Error) {
            if (['ValidationError', 'ConflictError', 'PayloadTooLargeError'].includes(error.name)) {
                throw error;
            }
        }

        // Log error without sensitive data
        console.error('Upsert manifest failed:', error instanceof Error ? error.message : 'Unknown error');
