...
export const createBookmark = async (
    ...
    try {
        ...
        if (!isValidBase64(input.nonce_content) ||
        ...
        const actualSize = nonceContent.length + ciphertextContent.length +
            nonceWrap.length + dekWrapped.length;

        // Validate size against provided size (optional check)
        if (input.size !== actualSize) {
            console.warn(`Size mismatch: provided=${input.size}, actual=${actualSize}`);
        }

        // Check size limit
        if (actualSize > config.items.maxSize) {
            const error = new Error(`Bookmark exceeds maximum size of ${config.items.maxSize} bytes`);
        ...
        const exists = await bookmarkRepository.bookmarkExists(input.item_id, vault.vaultId);
        ...
        console.log(`Bookmark created: item_id=${input.item_id}, vault_id=${vault.vaultId}`);
        ...
        if (error instanceof Error) {
            if (['ValidationError', 'ConflictError', 'PayloadTooLargeError', 'NotFoundError'].includes(error.name)) {
                throw error;
            }
        }

        // Log error without sensitive data
        console.error('Create bookmark failed:', error instanceof Error ? error.message : 'Unknown error');
...
export const getBookmark = async (
    ...
    try {
        ...
        // Fetch bookmark
        const bookmark = await bookmarkRepository.findBookmarkById(itemId, vault.vaultId);
        ...
        // Log error without sensitive data
        console.error('Get bookmark failed:', error instanceof Error ? error.message : 'Unknown error');
...
export const updateBookmark = async (
    ...
    try {
        ...
        const currentBookmark = await bookmarkRepository.findBookmarkById(itemId, vault.vaultId);
        ...
        // Log error without sensitive data
        console.error('Update bookmark failed:', error instanceof Error ? error.message : 'Unknown error');
...
export const deleteBookmark = async (
    ...
    try {
        ...
        await bookmarkRepository.softDeleteBookmark(itemId, vault.vaultId, input.deleted_at);

        console.log(`Bookmark soft-deleted: item_id=${itemId}, version=${input.version}`);
        ...
        // Log error without sensitive data
        console.error('Delete bookmark failed:', error instanceof Error ? error.message : 'Unknown error');
...
export const listBookmarks = async (
    ...
    try {
        ...
        const bookmarks = await bookmarkRepository.listBookmarksPaginated(vault.vaultId, {
        ...
        // Log error without sensitive data
        console.error('List bookmarks failed:', error instanceof Error ? error.message : 'Unknown error');
