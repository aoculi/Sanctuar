 ...
 */
export const uploadWrappedMasterKey = async (input: UploadWmkInput): Promise<void> => {
    ...
    try {
        ...
        console.error('WMK upload failed:', error instanceof Error ? error.message : 'Unknown error');

        if (error instanceof Error && error.message === 'Invalid wrapped master key format') {
            throw error;
        }
