...
function isValidBase64(value: string): boolean {
    try {
        return Buffer.from(value, 'base64').toString('base64') === value.replace(/\n|\r/g, '');
    } catch {
        return false;
    }
}
...
export const getTag = async (
    ...
    try {
        ...
        console.error('Get tag failed:', error instanceof Error ? error.message : 'Unknown error');
...
export const updateTag = async (
    ...
    try {
        ...
        if (!isValidBase64(input.nonce_content) || !isValidBase64(input.ciphertext_content)) {
        ...
        if (actualSize > config.items.maxSize) {
            const error = new Error(`Tag exceeds maximum size of ${config.items.maxSize} bytes`);
        ...
        // Log error without sensitive data
        console.error('Update tag failed:', error instanceof Error ? error.message : 'Unknown error');
...
export const listTags = async (
    ...
    try {
        ...
        // Log error without sensitive data
        console.error('List tags failed:', error instanceof Error ? error.message : 'Unknown error');
